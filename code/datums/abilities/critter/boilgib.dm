...
/datum/targetable/critter/boilgib
	...
	cast(atom/target)
		...
		bloodboil(dmg, get_turf(ow))
		ow.gib()

proc/bloodboil(damage, turf/T)
	for (var/turf/splat in view(2, T))
		if (prob(50))
			make_cleanable(/obj/decal/cleanable/blood/changeling, splat)
	...
	for (var/mob/M in view(3, T))
		if(iscarbon(M))
			...
			if (M.reagents)
				M.reagents.add_reagent("bloodc", damage, null, T100C)
